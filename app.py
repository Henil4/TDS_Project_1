...
async def generate_answer(question, relevant_results, max_retries=2):
    ...
    while retries < max_retries:    
        try:
            ...
            prompt = f"""
            You are an academic assistant. Answer the following question based **only** on the provided context. 

            👉 Important Rules:
            - Always display marks in **hundreds format**. 
            - For example: 
                - If the score is written as 11/10 in the context, you must write it as **110**.
                - If the score is written as 10/10, write it as **100**.
            - **Never use fractions, slashes, or percentages.**
            - Always use whole numbers in the final answer (like 110, 100, etc.).

            If you cannot answer the question based on the provided context, respond exactly with:
            "I don't have enough information to answer this question."

            👉 Strict Response Format:
            1. A detailed but concise answer, strictly using the hundreds format for scores.
            2. Sources:
            1. URL: [exact_url_1], Text: [brief quote or description]
            2. URL: [exact_url_2], Text: [brief quote or description]

            ⚡ Make sure:
            - You **copy the URLs exactly from the context** without any changes.
            - You list all relevant sources that support your answer.
